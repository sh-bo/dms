  ...
  const requestOtp = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        const employeeType = response.data.employeeType; // Assuming employeeType is returned in the response

        // Log employeeType for debugging
        console.log("Employee Type:", employeeType);

        // Check if employeeType is null
        ...
        // Store employeeType in local storage
  ...
  const handleLogin = async (e) => {
    ...
    if (isOtpRequested) {
      ...
      try {
        ...
        if (response.status === 200) {
          const { token, employeeType, name } = response.data;
